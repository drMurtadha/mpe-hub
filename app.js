(function(){
  ...
  function renderSummary(s){
    ...
    if(s.recent?.length)$("#activity-list").innerHTML=s.recent.map(item=>{const type=item.module==="asset"?"PA9":item.module==="mccb"?"TEST":"LOG";const klass=item.module==="asset"?"type-asset":item.module==="mccb"?"type-test":"type-log";const badge=/lengkap|lulus/i.test(item.status)?"success":"warning";return `<div class="activity-row"><span class="activity-type ${klass}">${type}</span><div><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.meta||"")}</small></div><span class="badge ${badge}">${escapeHtml(item.status||"")}</span></div>`;}).join("");
  }
